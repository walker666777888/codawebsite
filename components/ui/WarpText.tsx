...
const drawLine = (ctx, line, x, y, letterSpacing, color) => {
  const chars = Array.from(line);
  let cursor = x - measureLine(ctx, line, letterSpacing) / 2;

  chars.forEach((char, index) => {
    if (line === "Citizen Of Digital Age.") {
      ctx.fillStyle = index < 11 ? "#FFFFFF" : "#FF5C00";
    } else {
      ctx.fillStyle = color;
    }
...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  lines.forEach((line, index) => drawLine(ctx, line, width / 2, startY + index * lineHeight, letterSpacing, props.color));
