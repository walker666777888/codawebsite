...
export default function Navbar() {
  ...
  // The site is a light "paper" theme end-to-end, so the navbar always uses
  // dark ink. `scrolled` only swaps the bar surface: transparent at the top →
  // a floating glass pill once the user scrolls past the hero.
  ...
  return (
    <>
      ...
      >
        {/* Inner pill — position/size never change, only visual properties */}
        <div className={`w-full transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${scrolled ? "mx-4 max-w-[calc(100%-2rem)]" : "mx-0 max-w-full"}`}>
          ...
          {scrolled && (
            <span className="pointer-events-none absolute inset-x-6 top-0 h-px rounded-full" style={{ background: "linear-gradient(90deg,transparent,rgba(255,92,0,0.45) 50%,transparent)" }} />
          )}
              ...
              {NAV_ANCHOR_LINKS.map((link, i) => (
                ...
                >
                  {link.href ? (
                    ...
                    <button onClick={() => { setMobileMenuOpen(false); openForm(); }}
                      className="block font-instrument text-[52px] leading-[1.15] tracking-[-0.04em] text-[#0D0D0B]/70 hover:text-[#FF5C00] transition-colors duration-200 cursor-pointer">
                      {link.label}
                    </button>
                  )}
                </motion.div>
